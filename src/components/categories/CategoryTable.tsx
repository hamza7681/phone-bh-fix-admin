...
const CategoryTable = () => {
  ...
  const actions = (rowData: any) => {
    return (
      <div className='flex items-center gap-3'>
        <div
          onClick={() => router.push(`/brands/category-edit/${rowData._id}`)}
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res) {
        const filteredCategories = categories.filter((x: any) => x._id !== res.data.brand._id)
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        <Column header='Category Name' style={{ width: '50%' }} body={brandName}></Column>
        <Column style={{ width: '50%', textAlign: 'center' }} body={actions}></Column>
      </DataTable>
    </div>
  )
}
